...
namespace MP
{
  ...
  void MPAbstraction::CompleteSetup(void)
  {
    ...
    for(int r = 0 ; r < m_nrGoal; r++)
    {
      ...
      m_goalRegionToId.insert(std::make_pair(m_ridGoals[r], r));
    }

    for(int i = 0; i < GetNrRegions(); ++i)
    {
      m_regions[i]->m_colors.resize(m_nrGoal);
      m_regions[i]->m_distFromStarts.resize(m_nrRobot);
      m_regions[i]->m_distToGoals.resize(m_nrGoal);
      m_regions[i]->m_pathsToGoal.resize(m_nrGoal);
    }
    HCosts();
